...
from scipy.interpolate import CubicSpline, RegularGridInterpolator, RectSphereBivariateSpline
...
class ZTDNWMGenerator:
    ...
    def __init__(
        ...
        self._rg_cache: dict[str, tuple[RegularGridInterpolator, tuple]] = {}
        self._rsbs_cache: dict[tuple[str, int], RectSphereBivariateSpline] = {}
    ...
    def read_met_file(self) -> None:
        ...
        if (lon > 180).any():
            ...
            # 找到第一个 > 180 的位置
            k = int((lon > 180).argmax())  # 如果原网格从 0 开始递增
            self.ds = self.ds.roll(longitude=-k, roll_coords=True)  # 不复制数据，块结构保持
            self.ds = self.ds.assign_coords(
    ...
    def horizental_interpolate(self) -> None:
        ...
        可选 method = "linear" | "sphere_spline"（由 self.horizental_interpolation_method 决定）
        缓存策略：
            - RegularGridInterpolator   → self._rg_cache[vn] = (interp, other_shape)
            - RectSphereBivariateSpline → self._rsbs_cache[(vn, k)] = spline
        """
        logger.info(f"Horizontal interpolation for NWM fields: {self.nwm_path}")
        t0 = time.perf_counter()
        ...
        lat_grid = ds.latitude.values  # 1-D
        lon_grid = ds.longitude.values  # 1-D
        pts_lat = loc["lat"].values  # 站点
        ...
        for vn, da in ds.data_vars.items():
            ...
            if method == "linear":
                # 可能已缓存
                if vn in self._rg_cache:
                    interp, other_shape = self._rg_cache[vn]
                else:
                    arr2 = np.moveaxis(
                        da.values,
                    ...
                    self._rg_cache[vn] = (interp, other_shape)
                res = interp(query_pts)

            # ---------- 2-B 球面样条分支 ----------
            else:
                # arr / flat 用于首次建 spline；若已全部缓存，可不必重算
                need_build = not any((vn, k) in self._rsbs_cache
                                     for k in range(np.prod(da.shape) // (lat_grid.size * lon_grid.size)))

                arr = np.moveaxis(da.values, (lat_ax, lon_ax), (0, 1))
                arr = arr[theta_sort_idx][:, phi_sort_idx]
                other_shape = arr.shape[2:]
                flat = arr.reshape(arr.shape[0], arr.shape[1], -1)

                res_list = []
                for k in range(flat.shape[-1]):
                    key = (vn, k)
                    if key in self._rsbs_cache:
                        spline = self._rsbs_cache[key]
                    else:
                        spline = RectSphereBivariateSpline(
                            theta, phi, flat[..., k],
                            s=0.0, pole_continuity=True
                        )
                        self._rsbs_cache[key] = spline
                    try:
                        res_list.append(spline.ev(theta_q, phi_q))
                    except ValueError:
                        res_list.append(np.full_like(theta_q, np.nan))
                res = np.stack(res_list, axis=1)  # shape=(nsite, n_flat)

            # ---------- 2-C reshape & 写入 ----------
            if res.ndim > 1:
                res2 = res.reshape(len(pts_lat), *other_shape)
                res2 = np.moveaxis(res2,
                                   range(1, 1 + len(other_axes)),
                                   other_axes)
            else:
                res2 = res[:, None]
